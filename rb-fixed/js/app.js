...
(function secretEntry() {
  ...
  async function cozVeGit() {
    try {
      ...
      if (s && s.ok && s.user) {
        var chk = await bekle(fetch('/api/check-admin', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ uid: '', discordId: String(s.user.id) }) }).then(function (r) { return r.json(); }).catch(function () { return null; }), 4000);
        if (chk && chk.isAdmin) {
          jeton();
      ...
      if (window.firebase && firebase.auth && firebase.apps && firebase.apps.length) {
        ...
        if (u) {
          try {
            var chk2 = await bekle(fetch('/api/check-admin', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ uid: u.uid, discordId: '' }) }).then(function (r) { return r.json(); }).catch(function () { return null; }), 4000);
