...
function refreshForumAdmin() {
  var u = null;
  try { u = CUR(); } catch (e) {}
  var id = u && u.uid;
  if (!id) return Promise.resolve(false);
  if (window.__RB_ADMIN === true) return Promise.resolve(true);
  return fetch('/api/check-admin', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ uid: id }) })
...
function canModerate(targetRole) {
  const u = CUR();
  if (u && window.__RB_ADMIN === true) return true;
  if (!u) return false;
...
function renderLoginRetry() {
  try {
    view().innerHTML = '<div class="forum-wrap"><div class="rb-empty">Baglanti tamamlanamadi.<br><br><button class="rb-btn" onclick="location.reload()">Tekrar Dene</button> <a class="rb-btn" href="/api/auth/discord/start?next=' + encodeURIComponent('/forum') + '">Discord ile Giris</a></div></div>';
  } catch (e) {}
}
function renderLogin() {
  var ses = null;
  try { ses = (window.RBSession && window.RBSession.ok) ? window.RBSession : null; } catch (e) {}
  if (ses && window.RBLogin) {
    view().innerHTML = '<div class="forum-wrap"><div class="rb-empty">Discord hesabin baglaniyor, bekle...<br><small id="rb-bridge-state"></small></div></div>';
    var done = false;
    function bitir(ok) { if (done) return; done = true; if (ok) { try { route(); } catch (e) {} } else renderLoginRetry(); }
    try {
      window.RBLogin.init().then(function () {
        var d = null;
        try { d = window.RBLogin.durum(); } catch (e) {}
        if (d && d.firebase) { refreshForumAdmin().then(function () { bitir(true); }).catch(function () { bitir(true); }); }
        else bitir(false);
      }).catch(function () { bitir(false); });
    } catch (e) { bitir(false); }
    setTimeout(function () { bitir(false); }, 15000);
    return;
  }
  if (window.RBSession && window.RBSession.loading) {
    view().innerHTML = '<div class="rb-empty">Yukleniyor...</div>';
    setTimeout(function () { try { renderLogin(); } catch (e) {} }, 600);
    return;
  }
  try {
  ...
  view().innerHTML = '<div class="rb-empty">Discord\'a yonlendiriliyorsun...</div>';
}
...
async function renderMod() {
  if (!CUR() || myW() < 3) { location.hash = "#/"; return; }
  const _cu = CUR();
  const _isAdmin = _cu && (window.__RB_ADMIN === true || myW() >= 5);
  let rows = [];
  try {
...
function boot() {
  try {
    ...
    RBAuth.onAuth(function () { touchLastLogin(); try { refreshForumAdmin().then(function () { try { route(); } catch (e) {} }); } catch (e) { route(); } });
