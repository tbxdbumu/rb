...
export default async function handler(req, res) {
  ...
  if (op.endsWith('/contact') && req.method === 'POST') {
    ...
    try {
      const siteReferer = process.env.SITE_URL ? `${process.env.SITE_URL.replace(/\/+$/, '')}/` : 'https://risebunny.vercel.app/';
      const r = await fetch(
        `https://firestore.googleapis.com/v1/projects/${PROJECT}/databases/(default)/documents/messages?key=${FB_KEY}`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Referer: siteReferer },
          body: JSON.stringify({
            fields: {
