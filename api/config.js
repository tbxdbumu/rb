...
const configDocUrl = (id) =>
  `https://firestore.googleapis.com/v1/projects/${FB_PROJE}/databases/(default)/documents/launcher_configs/${id}?key=${FB_ANAHTAR}`;
...
export default async function handler(req, res) {
  ...
  if (op.endsWith('/configs') && req.method === 'GET') {
    try {
      const r = await fetch(
        `https://firestore.googleapis.com/v1/projects/${FB_PROJE}/databases/(default)/documents/launcher_configs?key=${FB_ANAHTAR}&orderBy=createdAt desc&pageSize=50`,
  ...
  if (op.endsWith('/configs') && req.method === 'POST') {
    ...
    try {
      ...
      // Bot'a bildir (sahip loguna düşer)
      if (bBase && process.env.BOT_API_SECRET) {
        try {
  ...
  // Config indir (launcher - sadece onaylanmış)
  if (op.match(/^\/configs\/[^/]+$/)) {
